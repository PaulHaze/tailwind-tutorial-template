import React from 'react';

export function Home() {
  return (
    <div className="w-full pt-10 bg-gradient-to-b from-indigo-100 to-violet-200 px-4">
      <div className="flex flex-col items-center justify-center max-w-xs h-40 mx-auto bg-white border rounded shadow-lg text-center">
        <h5 className="font-semibold text-sky-300">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
      <div className="flex items-center justify-center w-8/12 h-40 mx-auto bg-white border rounded shadow-lg px-4 text-center">
        <h5 className="font-semibold text-coolgray-700">
          Delete this and start coding
        </h5>
      </div>
    </div>
  );
}
